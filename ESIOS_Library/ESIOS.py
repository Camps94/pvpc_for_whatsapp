...
class ESIOS(object):
    ...
    def __get_query_json__(self, indicator, start_str, end_str):
        ...
        url = 'https://api.esios.ree.es/indicators/' + indicator + '?start_date=' + start_str + '&end_date=' + end_str +'&geo_ids[]=8741'
    ...
    def get_pvpc_results(self, token):
        start_ = datetime.datetime.now() + datetime.timedelta(hours=6)
        ...
        pvpc = ''
        minimun_values = df['Término de facturación de energía activa del PVPC 2.0TD'].nsmallest(1)
        maximun_values = df['Término de facturación de energía activa del PVPC 2.0TD'].nlargest(1)
        prices = []
        for index, row in df.iterrows():
            index = index[11:13]
            price = round(row[0]/1000, 2)
            price = format(price, '.2f')
            if row[0] == minimun_values.iloc[0] or row[0] == maximun_values.iloc[0]:
                price = '*' + str(price) + '*'
