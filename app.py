...
def updateDDBB():
    ...
    try:
        ...
        if msg == "alta":
            ...
            action = 'FALSE'
            resp.message("¡Vaya, siento que no te haya sido útil! Acabas de desactivar el servicio de alarma diario del PVPC.")
            sql_query = "INSERT INTO users (name, status) VALUES ('{}', '{}') ON CONFLICT (name) DO UPDATE SET status = '{}';".format(number, action, action)        
            cursor.execute(sql_query)
            connection.commit()

        else:
            resp.message("""Por favor, envíe la palabra *ALTA* o *BAJA* para darse de ALTA/BAJA en el servicio de alarma del PVPC. Recibirá un mensaje cada día a las 20:30 GMT+01:00 con el PVPC del próximo día.""")
